...
5. `litt export ID /tmp/litt-ID.png` exports the supporting screenshot for inspection.
...
Recordings may be stored in HEVC chunks. Always use `litt export` to retrieve a frame; do not assume a `frames/ID.jpg` file exists. PNG export avoids another lossy JPEG encoding.
