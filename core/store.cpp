...
struct Stmt {
  ...
  void bind(int n, double v) { sqlite3_bind_double(p, n, v); }
  void bind(int n, long long v) { sqlite3_bind_int64(p, n, v); }
  int step() {
...
Store::Store(fs::path root) : root_(std::move(root)) {
  ...
  exec("CREATE TABLE IF NOT EXISTS frames(id INTEGER PRIMARY KEY "
       ...
       "VALUES('delete',old.id,old.title,old.text,old.app);END;PRAGMA "
       "user_version=1;");
...
json Store::frames(std::string query, std::string bundle, double from,
                   double to, int limit, int offset) {
  std::lock_guard lock(mutex_);
...
json Store::stats() {
  ...
  Stmt s(
      db_,
      "SELECT count(*),coalesce(sum(bytes),0),min(time),max(time) FROM frames");
  s.step();
...
void Store::erase(long long id) {
  ...
  fs::remove(path, ec);
  if (ec)
    throw std::runtime_error("Index removed, but image deletion failed: " +
                             ec.message());
}
...
void Store::settings(const json &changes) {
  ...
  if (!all["interval"].is_number_integer() || all["interval"] < 1 ||
      all["interval"] > 30)
...
bool ChangeGate::changed(const std::vector<unsigned char> &pixels,
                         const std::string &context, double time) {
  if (pixels.empty())
    return false;
  bool take = previous_.size() != pixels.size() || context != context_ ||
              time - saved_ >= 30 || time < saved_;
  if (!take) {
    size_t changed = 0;
    double total = 0;
    ...
    take =
        total / pixels.size() > 0.45 || double(changed) / pixels.size() > 0.005;
  }
  if (take) {
    previous_ = pixels;
    context_ = context;
