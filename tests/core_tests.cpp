...
int main() {
  auto path = fs::temp_directory_path() /
              ("litt-tests-" + std::to_string(getpid()));
  fs::remove_all(path);
  try {
    {
      Store store(path);
      ...
      check(store.frames("", "", f.time + 1, f.time + 2).empty(),
            "Time filtering");
      ...
      store.settings(
          {{"interval", 5}, {"excluded", json::array({"private.app"})}});
    ...
    }
    {
      ...
      check(reopened.settings()["excluded"][0] == "private.app",
