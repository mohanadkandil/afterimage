  ...
  NSData *data =
      [NSJSONSerialization dataWithJSONObject:obj
                                      options:NSJSONWritingFragmentsAllowed
                                        error:nil];
  return data ? json::parse(std::string((const char *)data.bytes, data.length))
              : json();
}
static fs::path defaultRoot() {
  ...
  if (env && *env)
    return env;
  return fs::path(str(NSHomeDirectory())) /
         "Library/Application Support/Litt";
}
static CGImageRef loadImage(NSString *path) {
  CGImageSourceRef src = CGImageSourceCreateWithURL(
      (__bridge CFURLRef)[NSURL fileURLWithPath:path], nil);
  if (!src)
    return nullptr;
...
static std::vector<unsigned char> grayscale(CGImageRef im) {
  ...
  CGContextRef ctx =
      CGBitmapContextCreate(p.data(), 160, 90, 8, 160, cs, kCGImageAlphaNone);
...
static Prepared prepare(CGImageRef image, Frame f) {
  @autoreleasepool {
    ...
    if (![handler performRequests:@[ request ] error:&error])
      throw std::runtime_error("Text recognition failed: " +
    ...
    CGImageDestinationAddImage(
        dest, image, (__bridge CFDictionaryRef) @{
          (__bridge NSString *)
...
static int cli(int argc, char **argv) {
  try {
    ...
    if (command == "stats")
      ...
      result = {
          {"screenRecordingPermission", (bool)CGPreflightScreenCaptureAccess()},
...
- (void)applicationDidFinishLaunching:(NSNotification *)n {
  (void)n;
  worker =
      dispatch_queue_create("app.litt.processing", DISPATCH_QUEUE_SERIAL);
  ...
  [appMenu addItemWithTitle:@"Quit Litt"
...
- (void)start {
  ...
  timer =
      [NSTimer scheduledTimerWithTimeInterval:preferences["interval"].get<int>()
                                       target:self
...
- (void)tick:(id)sender {
  ...
  if (!recording || busy)
    return;
  NSRunningApplication *focused =
      NSWorkspace.sharedWorkspace.frontmostApplication;
  std::string bundle = str(focused.bundleIdentifier),
              name = str(focused.localizedName);
  // Avoid capturing our UI or any part of the desktop while an excluded app is
  // focused.
  if (bundle == "app.litt.local" ||
      std::find(preferences["excluded"].begin(), preferences["excluded"].end(),
                json(bundle)) != preferences["excluded"].end()) {
    captureState = bundle == "app.litt.local"
                       ? "Waiting · Litt is focused"
                             ...
                             onScreenWindowsOnly:YES
                               completionHandler:^(SCShareableContent *content,
                                 ...
                                 dispatch_async(dispatch_get_main_queue(), ^{
                                   ...
                                   for (SCRunningApplication *a in content
                                            .applications) {
                                   ...
                                   [SCScreenshotManager
                                       captureImageWithFilter:filter
                                            ...
                                            completionHandler:^(CGImageRef im,
                                                                NSError *err) {
                                              if (!im) {
                                                std::string msg =
...
- (json)state {
  ...
  for (NSRunningApplication *a in NSWorkspace.sharedWorkspace
           .runningApplications) {
    if (a.activationPolicy == NSApplicationActivationPolicyRegular &&
        a.bundleIdentifier &&
        ![a.bundleIdentifier isEqualToString:@"app.litt.local"])
      apps.push_back({{"name", str(a.localizedName)},
                      {"bundle", str(a.bundleIdentifier)}});
  }
  j["runningApps"] = apps;
  return j;
}
...
- (void)performAction:(NSDictionary *)body {
  ...
  try {
    ...
    if (action == "state")
      ...
      [self reply:request
           result:archive->frames(
                      args.value("query", ""), args.value("app", ""),
      ...
      panel.nameFieldStringValue =
          ns("litt-" + std::to_string(args.at("id").get<long long>()) +
             ".jpg");
